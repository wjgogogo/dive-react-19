...
> 快速结论：初次挂载由“创建根 → 触发渲染 → 调度 → 构建 Fiber → 完成阶段 → 提交阶段”串联完成，React 19 默认并发渲染（ConcurrentRoot），提交阶段分 Before Mutation / Mutation / Layout 三步。

:::tip 前置阅读
- 如何本地调试源码：见《如何调试 React 源码？》；快速启动：`pnpm install && pnpm react:dev`
- 并发调度背景：见《Scheduler 调度器》
:::
...
初次挂载流程涉及这三个部分的紧密协作，形成从组件到实际 UI 的转化过程。
...
这里建立了 FiberRoot 和 RootFiber 的相互引用，形成应用的基础结构。
...
其中，`requestUpdateLane` 为更新分配优先级（Lane），这是 React 19 并发模式的核心。
...
2. `completeUnitOfWork`：在没有更多子节点时完成节点处理，然后处理兄弟节点或返回父节点
