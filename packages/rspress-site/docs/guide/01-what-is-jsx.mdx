...
在开发模式下，即使用 `@babel/plugin-transform-react-jsx-development`。会将绝大部分 JSX 转换为 `jsxDEV` 函数调用。
...
- `config` 中包含除 `chidren` 的所有 props，包含 `key`，`ref`
...
6. 性能优化：
   - 代码中包含一些性能优化措施，如尽可能重用原始的 props 对象。
7. 特性开关：
   - 代码中使用了多个特性开关（如 enableRefAsProp、disableStringRefs 等），用于控制不同的 React 特性。

:::info
`jsxDEV` 函数代码中包含许多仅在开发环境（**DEV**）下运行的检查和警告。这些检查帮助开发者发现潜在的问题，如类型错误、key 使用不当等。
...
function jsxDEVImpl(
  ...
  if (__DEV__) {
    ...
    // 现阶段，依旧会从 config 中获取 key 值，但是为了下一阶段准备，会警告 key 使用不当
    ...
    // 下面的注释提到了对 <div {...props} key="Hi" or <div key="Hi" {...props} /> 两种情况的处理还有点问题
...
```js title="react/packages/shared/ReactFeatureFlags.js"
...
Babel 会将 JSX 的 props 使用一个新的对象来承载，在以前的版本中，`jsxDEV` 还是会创建一个新的对象，用于承载剔除掉 `ref`，`key`等保留属性，而现在，会尽可能重用这个对象，以减少内存分配和垃圾回收的负担。
...
:::tip
ReactElement 元素 和 Fiber 节点是不同的，后文会详解介绍 Fiber 节点，注意不要混淆了。
:::

#### createElement 函数

上面提到有些特殊情况，JSX 会被转换成 `createElement` 函数调用。下面来看下 `createElement` 函数的实现。
...
export function createElement(type, config, children) {
  ...
  if (config != null) {
    ...
    // 将 config 中除 key之外的属性添加到 props 对象中，由于每次都会构建新的 props 对象，性能会差点
  ...
  // 与 jsxDEV 不同的是，少了特性开关，所以组件的 defaultProps，会和以前版本一样其合并到 props 中
  // Resolve default props
  if (type && type.defaultProps) {
    const defaultProps = type.defaultProps;
...
:::tip
我们偶尔可能也会用到[`React.cloneElement`](https://react.dev/reference/react/cloneElement) 函数，它主要用于克隆元素，并进行一些扩展。核心代码和 `React.createElement` 函数流程大致一致，不再赘述。
:::
