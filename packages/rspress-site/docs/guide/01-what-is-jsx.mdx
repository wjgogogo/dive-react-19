...
以 [@babel/preset-react](https://babeljs.io/docs/babel-preset-react) 7.25.7 版本为准，主要包含以下两个插件：
...
:::tip
我们偶尔可能也会用到 [`React.cloneElement`](https://react.dev/reference/react/cloneElement) 函数，它主要用于克隆 ReactElement 元素，并进行一些扩展。核心代码流程和 `React.createElement` 函数大致一致，不再赘述。
:::
