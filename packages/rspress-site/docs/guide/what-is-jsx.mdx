...
> 快速结论：JSX 是语法扩展，需经编译器转换为函数调用（automatic: `jsx/jsxs/jsxDEV`；classic: `React.createElement`），最终产出 `ReactElement`。React 19 起，`key` 与其他 props 分离、函数组件不再支持 `defaultProps`，`ref` 行为调整为更显式且开发期具备告警能力。

我们使用 React 时最大的感受是它让我们能够用一种更直观、更声明式的方式来描述用户界面，即 JSX。JSX 的语法类似于 HTML，但它并不是 HTML，而是 JavaScript 的一种语法扩展。
...
本节以 Babel 为准（SWC/ESBuild/TS 配置思路一致）。
...
目前 `automatic` 是主流 runtime，无需手动引入 `React`。
...
而采用 `classic` runtime，还必须手动引入 `React`，否则会报错。
...
无论是 `automatic` 还是 `classic`，本质都是将 JSX 转换成函数调用，而函数执行后最终会返回一个 `ReactElement` 数据结构。

比如上面的 `element` 就是 `ReactElement` 数据结构，大概长这样：
...
- 函数：表示类、函数组件
- Symbol（或者特殊字符串 polyfill，主要针对 React 内置组件）：如 `Fragment`、`StrictMode` 等

Babel 会根据 `type` 类型，选择不同的转换逻辑。如果是原生标签（以小写开头的字符串），会转换为 `string` 类型，否则保持原样。
...
const element = /*#__PURE__*/ _jsxDEV(
  ...
  // !callout[/key/]  key 会被单独提出来，便于提高后续 React 解析性能
  "key",
  false,
  {
    fileName: _jsxFileName,
...
:::info
现阶段 React 仍会在运行时从 `config` 再解析一次 `key`，并抛出 warning 提示应将 `key` 单独指定。未来可能完全停止从 `config` 解析 `key`。
:::
...
:::note
实践建议：将 `key` 放在展开属性之前（如 `<div key="Hi" {...props} />`），避免被编译为 `React.createElement`，通常可获得更好性能。Babel 的行为未来可能调整。
:::
...
function App() {
  ...
  return /*#__PURE__*/ React.createElement(
    ...
    // !callout[/element/] 从第三个参数开始，后续的入参都会被视为是 children 的一项
    element,
    " ",
...
开发模式相比生产模式，会额外注入调试信息（如 `config.__self`、`config.__source`）。
...
function jsxDEVImpl(
  ...
  if (__DEV__) {
    ...
    // !className(1:26) border-2! border-blue-300! rounded-sm
    ...
    if (!("key" in config)) {
      ...
      for (const propName in config) {
        ...
        // !callout[/\(.*\)/] 不会对 ref 做特殊处理，所以 ref 不会再作为保留属性，而是会保留在 props 中
        if (propName !== "key") {
    ...
    // !callout[/disableDefaultPropsExceptForClasses/] 在 19 中，disableDefaultPropsExceptForClasses 为 true, 所以不会再将 defaultProps 合并到 props 中
    ...
    // !callout[/key/] 对从 props 中获取的 key 做特殊处理，添加开发环境警告
    if (key) {
      const displayName =
    ...
    // !callout[/ReactElement/] 最后调用 ReactElement 函数创建 React 元素
    return ReactElement(
      type,
      key,
      self,
...
function ReactElement(
  ...
  if (__DEV__) {
    ...
    // !callout[/ref/] 对 ref 做特殊处理，添加开发环境警告
    if (ref !== null) {
...
Babel 会将 JSX 的 props 使用一个新对象来承载，_在以前的版本中，`jsxDEV` 总是会创建一个新的对象，用于承载剔除掉 `ref`、`key` 等保留属性_，**而现在，会尽可能重用这个对象，以减少内存分配和垃圾回收的负担**。
...
:::tip
ReactElement 元素和 Fiber 节点是不同的，后文会详解介绍 Fiber 节点，注意不要混淆。
:::
...
export function createElement(type, config, children) {
  ...
  // !callout[/config/] config 中包含除 children 的所有 props，包含 key，ref
  if (config != null) {
    if (__DEV__) {
      // !mark
      // 对旧的 JSX 转换做警告
...
- JSX 本质是编译期语法糖，产出 `ReactElement`。
- automatic 下使用 `jsx/jsxs/jsxDEV`，classic 使用 `React.createElement`。
- 开发模式注入 `__source`/`__self` 等调试信息；生产模式剔除。
- React 19 对 `key/ref/defaultProps` 行为做了更一致、更高性能的调整。
